...
void unittesting(int L = 55) {
  ...
  cout << "percentage of random numbers above average = "
       << ((double)counter / times) << endl;
}
...
int main(int params, char **args) {
  ...
  for (int i = 1; i < params - 1; ++i) {
    ...
    if (strcmp(args[i], "--groupby") == 0) {
      ...
      if (params - i > 1) {
        ...
        cerr << "--groupby expects a string"
             << endl; // Voir comment verifier une cha�e en c++
        return -2;
      }
    }
    ...
    if (strcmp(args[i], "-s") == 0) {
      s = atoi(args[++i]);
      if (verbose == true)
        cout << "Memory size or number of bitmaps: " << s << endl;
    }

    if (strcmp(args[i], "-l") == 0) {
      L = atoi(args[++i]);
      if (verbose == true)
        cout << "Number of bits for hashing: " << L << endl;
    }
    ...
    if (strcmp(args[i], "--gibbons") == 0) {
      method = gibbons;
      if (verbose == true)
        cout << "Counting by the Gibbons' method" << endl;
    }
    if (strcmp(args[i], "--baryossef") == 0) {
      method = baryossef;
      if (verbose == true)
        cout << "Counting by the Bar-Yossef' method" << endl;
    }
    if (strcmp(args[i], "--adaptive") == 0) {
      method = adaptive;
      if (verbose == true)
        cout << "Counting by the Adaptive counting' method" << endl;
    }
    if (strcmp(args[i], "--stogibbons") == 0) {
      method = stogibbons;
      stogibbonscount = atoi(args[++i]);
      if (verbose == true)
        cout << "Counting by the Sto. Gibbons' method" << endl;
    }
    if (strcmp(args[i], "--stosumgibbons") == 0) {
      method = stosumgibbons;
      stogibbonscount = atoi(args[++i]);
      if (verbose == true)
        cout << "Counting by the Sto. Sum Gibbons' method" << endl;
    }
    if (strcmp(args[i], "--stoweightedsumgibbons") == 0) {
      method = stoweightedsumgibbons;
      stogibbonscount = atoi(args[++i]);
      if (verbose == true)
        cout << "Counting by the Sto. Weighted Sum Gibbons' method" << endl;
    }
    if (strcmp(args[i], "--counting") == 0) {
      method = counting;
      if (verbose == true)
        cout << "Counting by the probalistic counting" << endl;
    }
    ...
    if (strcmp(args[i], "--loglog") == 0) {
      method = loglog;
      if (verbose == true)
        cout << "Counting by the loglog counting" << endl;
    }

    if (strcmp(args[i], "--readfile") == 0) {
      method = readfile;
      if (verbose == true)
        cout << "Just reading..." << endl;
    }
    if (strcmp(args[i], "--readhash") == 0) {
      method = readhash;
      if (verbose == true)
        cout << "Just reading and hashing..." << endl;
    }
  }
  ...
  case gibbons:
    if (eightbits)
      answer =
          GibbonsTirthapura<char>(filein, groupby, N, L, s, delimiter, rng);
      ...
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
                "slow"
             << endl;
      answer =
          GibbonsTirthapura<string>(filein, groupby, N, L, s, delimiter, rng);
    }
    break;

  case stogibbons:
    if (eightbits)
      answer = StoGibbonsTirthapura<char>(filein, groupby, N, L, s,
                                          stogibbonscount, delimiter, rng);
    else if (thirtytwobits)
      answer = StoGibbonsTirthapura<int>(filein, groupby, N, L, s,
                                         stogibbonscount, delimiter, rng);
    else {
      if (verbose == true)
        cout << "Assuming attribute values are strings???" << endl;
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
                "slow"
             << endl;
      answer = StoGibbonsTirthapura<string>(filein, groupby, N, L, s,
                                            stogibbonscount, delimiter, rng);
    }
    break;
  ...
  case stosumgibbons:
    if (eightbits)
      answer = StoSumGibbonsTirthapura<char>(filein, groupby, N, L, s,
                                             stogibbonscount, delimiter, rng);
    else if (thirtytwobits)
      answer = StoSumGibbonsTirthapura<int>(filein, groupby, N, L, s,
                                            stogibbonscount, delimiter, rng);
    else {
      if (verbose == true)
        cout << "Assuming attribute values are strings???" << endl;
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
                "slow"
             << endl;
      answer = StoSumGibbonsTirthapura<string>(filein, groupby, N, L, s,
                                               stogibbonscount, delimiter, rng);
    }
    break;
  case stoweightedsumgibbons:
    if (eightbits)
      answer = StoWeightedSumGibbonsTirthapura<char>(
          filein, groupby, N, L, s, stogibbonscount, delimiter, rng);
    else if (thirtytwobits)
      answer = StoWeightedSumGibbonsTirthapura<int>(
          filein, groupby, N, L, s, stogibbonscount, delimiter, rng);
    else {
      if (verbose == true)
        cout << "Assuming attribute values are strings???" << endl;
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
                "slow"
             << endl;
      answer = StoWeightedSumGibbonsTirthapura<string>(
          filein, groupby, N, L, s, stogibbonscount, delimiter, rng);
    }
    break;
  ...
  case counting:
    if (eightbits)
      answer = StoProbabilisticCounting<char>(filein, groupby, N, L, s,
                                              delimiter, rng);
    else if (thirtytwobits)
      answer = StoProbabilisticCounting<int>(filein, groupby, N, L, s,
                                             delimiter, rng);
    else {
      if (verbose == true)
        cout << "Assuming attribute values are strings???" << endl;
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
                "slow"
             << endl;
      answer = StoProbabilisticCounting<string>(filein, groupby, N, L, s,
                                                delimiter, rng);
    }
    break;
    ...
    if (eightbits)
      ...
      if (verbose == true)
        cout << "Please try --8bits or --32bits instead, this is going to be "
             ...
             << endl;
      answer =
          AdaptiveCounting<string>(filein, groupby, N, L, s, delimiter, rng);
    }
    break;
