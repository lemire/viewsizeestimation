...
/* The upper 32 bits will be count, the lower are the traditional "number of
 * trailing zeros" */
...
template <class T>
uint64 GibbonsTirthapura(istream &in, vector<string> &groupby, int &N,
                         int L = 19, int storagelimit = 512,
                         char delimiter = ',', int rng = rand_rng) {
  map<vector<T>, uint64> buffer;
  const uint n = groupby.size(); // number of dimensions
  StringConverter<T> sc;

  // Estimating simpOuie functions on the union of data streams by Gibbons and
  // Tirthapura
    ...
    if (projection == 0) {
      ...
      if (it == buffer.end()) {
        ...
        while ((buffer.size() > (uint)storagelimit)) {
          ...
          mask = (1 << (t)) - 1;
          assert(t < (uint)L);
          for (typename map<vector<T>, uint64>::iterator j = buffer.begin();
               j != buffer.end();)
            // if( (j->second & 0xffffffff)  < t) buffer.erase(j++);
            if ((j->second) < t)
              buffer.erase(j++); // & 0xffffffff
            else
              ++j;
        }
...
template <class T>
uint64 ProbabilisticCounting(istream &in, vector<string> &groupby, int &N,
                             int L = 19, const char &delimiter = ',',
                             int rng = rand_rng) {
...
template <class T>
uint64 LogLogCounting(istream &in, vector<string> &groupby, int &N, int L = 32,
                      int m = 64, const char &delimiter = ',',
                      int rng = rand_rng) {
  const float pi = 3.14;
  const double log2 = log((double)2);

  vector<uint> bucket(m, 0); // initialized to zero by constructor
  const uint kbits = (uint)round(
      log((double)m) / log2); // k-bits to extract from the hashed values
  assert(m == 1 << kbits);
  assert(kbits < (uint)L);
  const double alpha =
      (double)(0.3970 -
               (2 * pi * pi + log2 * log2) /
                   (48 * m)); // for checking  alpha(64) must be equal to 0.783
  const uint n = groupby.size(); // number of dimensions
  StringConverter<T> sc;
  /*for (int j=0; j < m; ++j)
    bucket[j]=0;*/
  ...
  vector<string> attribute_list; // Pour stocker les attributs de dimension et
                                 // leur valeur de chaque faits
  vector<uint> dimindice; // Pour stocker les indices des colonnes correspondant
                          // aux attributs du group by

  N = 0;
  // Récupérer les indices des colones des dimensions
  getline(in, line); // La première ligne du fichier de donnès contient les noms
                     // des dimensions
  string delimitertostring(1, delimiter);
  split(line, attribute_list, delimitertostring);
  ...
  while (in.getline(chline, MAXLENGTHOFALINE, delimiter)) {
    ...
    const uint firstkbits =
        hash >> (uint)(L - kbits); // Extracting first k-bits b0b1...bk-1
    uint leftposition =
        lefttrailingzeros((hash << kbits), L) +
        1; // the position of the left 1-bit in bk... (rank from 1)
  ...
  if (m >= 64) {
    ...
    double alpham = pow((-1 * pi / (exp(lgamma(1 + (1.0 / m))) * sin(pi / m))) *
                            ((pow(2, 1.0 / m) - 1) / log2),
                        -1 * m);
...
template <class T>
uint64 StoProbabilisticCounting(istream &in, vector<string> &groupby, int &N,
                                int L = 19, int nbofbitmap = 64,
...
template <class T>
int ReadFactsFromFile(istream &in, vector<string> &groupby, int &N,
                      const char &delimiter = ',') {
  const uint n = groupby.size(); // number of dimensions
  StringConverter<T> sc;
  string line; // Read a line of data
...
template <class T>
uint64 ReadHashFact(istream &in, vector<string> &groupby, int &N, int L = 19,
                    char delimiter = ',', int rng = rand_rng) {
  // map< vector<T>, uint64> buffer;
  const uint n = groupby.size(); // number of dimensions
  StringConverter<T> sc;
...
template <class T>
uint64 AdaptiveCounting(istream &in, vector<string> &groupby, int &N,
                        int L = 32, int m = 64, const char &delimiter = ',',
                        int rng = rand_rng) {
  const float pi = 3.14;
  const double log2 = log((double)2);
  ...
  assert(kbits < (uint)L);
  const double alpha =
      (double)(0.3970 - (2 * pi * pi + log2 * log2) /
                            (48 * m)); // alpha(64) must be equal to 0.783
  const uint n = groupby.size();       // number of dimensions
  StringConverter<T> sc;
  TupleFactHash<T> cph(n, L, rng);
  ...
  if (((double)empty / m) >= 0.051) {
    ...
    if (m >= 64) {
      ...
      double alpham =
          pow((-1 * pi / (exp(lgamma(1 + (1.0 / m))) * sin(pi / m))) *
                  ((pow(2, 1.0 / m) - 1) / log2),
              -1 * m);
      ans = (int)(alpham * m * pow(2, (double)(s / m)));
    }
