...
public:
  ...
  // seed by seeking in file: far, but not too far.  Stay in first 2MB. But
  // amplify seeds
...
template <class T> class RandomHasher {
public:
  RandomHasher() : wordsize(-1), mMap(), logrm(-1), rngKind(rand_rng) {}

  RandomHasher(int mywordsize, int rng = rand_rng)
      : wordsize(mywordsize), mMap(), logrm(0), rngKind(rng) {
    cout
        << "[WARNING] USING SLOW RANDOM HASHER, consider switching to integers."
        << endl
        << endl
        << endl;
    unsigned int rm;
    ...
    case rand_rng:
      ...
      break;
    case mt_rng:
      rm = randomSource.rand_max();
      break;
    // case mars_rng: rm = randomSourceMZ.rand_max(); break;
    case noisefile_rng:
      rm = randomSourceFile.rand_max();
      break;
    default:
      cerr << "rngKind = " << rngKind << " unknown" << endl;
      rm = 57;
    }

    while (rm > 0) {
      rm = rm >> 1;
      ++logrm; // Nombre de bits d�cal�s log rm
    }
  }
  ...
  void forcehash(T t, uint64 hashvalue) { mMap[t] = hashvalue; }
  ...
  uint64 generateRandomInteger() {
    ...
    while (bits != 0) {
      ...
      case rand_rng:
        ...
        break;
      case mt_rng:
        randombits = randomSource();
        break;
      // case mars_rng: randombits = randomSourceMZ(); break;
      case noisefile_rng:
        randombits = randomSourceFile();
        break;
      default:
        cerr << "rngKind=" << rngKind << " unk in hash" << endl;
      }
      if (offset < 0) {
  ...
  int getWordSize() const { return wordsize; }
...
template <> class RandomHasher<string> {
public:
  RandomHasher<string>() : wordsize(-1), mMap(), logrm(-1), rngKind(rand_rng) {}

  RandomHasher<string>(int mywordsize, int rng = rand_rng)
      : wordsize(mywordsize), mMap(), logrm(0), rngKind(rng) {
    unsigned int rm;
    switch (rngKind) {
    case rand_rng:
    ...
    while (rm > 0) {
      rm = rm >> 1;
      ++logrm; // Nombre de bits 
    }
  }

  uint64 hash(string t) {
    std::unordered_map<string, uint64>::iterator p = mMap.find(t);
    if (p != mMap.end())
      return p->second;
    return mMap[t] = generateRandomInteger();
  }
...
private:
  int wordsize;
  std::unordered_map<string, uint64> mMap;
  int logrm;
  int rngKind;
};

template <> class RandomHasher<int> {
public:
  RandomHasher<int>() : wordsize(-1), mMap(), logrm(-1), rngKind(rand_rng) {}

  RandomHasher<int>(int mywordsize, int rng = rand_rng)
      : wordsize(mywordsize), mMap(), logrm(0), rngKind(rng) {
    unsigned int rm;
    switch (rngKind) {
    case rand_rng:
  ...
  uint64 hash(int t) {
    std::unordered_map<int, uint64>::iterator p = mMap.find(t);
    if (p != mMap.end())
      return p->second;
    return mMap[t] = generateRandomInteger();
  }
...
private:
  int wordsize;
  std::unordered_map<int, uint64> mMap;
  int logrm;
  int rngKind;
};

template <> class RandomHasher<char> {
public:
  RandomHasher<char>() : wordsize(-1), mMap(), logrm(-1), rngKind(rand_rng) {}

  RandomHasher<char>(int mywordsize, int rng = rand_rng)
      : wordsize(mywordsize), mMap(), logrm(0), rngKind(rng) {
    unsigned int rm;
    switch (rngKind) {
    case rand_rng:
  ...
  uint64 hash(char t) { return mMap[(unsigned char)t]; }
 ...
 */
template <class T> class TupleFactHash {
public:
  TupleFactHash(int myn, int mywordsize = 19, int rng = rand_rng)
      : n(myn), wordsize(mywordsize), rh(0) {
    for (int k = 0; k < n; ++k)
  ...
  int hashSize() const { return wordsize; }
...
{
public:
        RandomTupleHasher(int myn, int mywordsize=19, int rng = rand_rng) :
n(myn), wordsize(mywordsize), rh(mywordsize, rng){}
    ...
    uint64 slow(const deque<T>& chars)
        {
                return rh.hash(chars);     // I think that == for deques will do
the right thing.
                                 // Stroustrup says they're like vectors and
vectors
