...
*/
template <class T>
uint64 BarYossef(istream & in, vector<string> & groupby, int& N, int L=19, uint storagelimit=512, char& delimiter = ',', int rng = rand_rng) 
{
  //int nbofbitmap = 32;
...
*******************************************/
    ...
    struct adder : public unary_function<uint64, void>
    {
      adder() : sum(0) {}
      uint64 sum;
...
*/
template <class T>
uint64 StoGibbonsTirthapura(istream & in, vector<string> & groupby, int& N, int L=19, int storagelimit=512, int nbofbitmap=32, char& delimiter = ',', int rng = rand_rng) 
{
  //int nbofbitmap = 32;
  ...
  vector<uint> mask(nbofbitmap,0);// = (1 << (t)) - 1;   // which is initially 0
  string line; //Read a line of data
  vector<T> attribute_value(n); 
  vector<string> attribute_list;//Pour stocker les attributs de dimension et leur valeur de chaque faits
  vector<unsigned int> dimindice;//Pour stocker les indices des colonnes correspondant aux attributs du group by
  ...
  while (in.getline(chline,MAXLENGTHOFALINE,delimiter))
  {
    ...
    if (projection == 0) 
    {
      typename map< vector<T>,uint64>::iterator it = buffer[alpha].find(attribute_value); 
      if (it == buffer[alpha].end()) 
      {
        //buffer[attribute_value] = (1LL << 32) | trailingzeros(oldhash,L);
        buffer[alpha][attribute_value] = trailingzeros(oldhash,L);//(1LL << 32) |
        while( (buffer[alpha].size() >(uint) storagelimit/nbofbitmap) ) 
        {
          t[alpha]+= 1;
          mask[alpha] = (1 << (t[alpha])) - 1;
          assert(t[alpha] < (uint)L);
          for(typename map<vector<T>, uint64>::iterator j = buffer[alpha].begin(); j != buffer[alpha].end();) 
          //if( (j->second & 0xffffffff)  < t) buffer.erase(j++); 
          if( (j->second )  < t[alpha]) buffer[alpha].erase(j++); // & 0xffffffff
          else ++j;
        }
  ...
  //int estimate = 0;
  if(nbofbitmap <1) cerr << "oops! expect garbage!!!" <<endl;
  if(nbofbitmap == 1) return estimates[0];
  if(nbofbitmap % 2 == 0)
    return (int)round((estimates[nbofbitmap/2-1]+estimates[(nbofbitmap/2)])/2.0);
  return estimates[nbofbitmap/2];
}
template <class T>
uint64 StoSumGibbonsTirthapura(istream & in, vector<string> & groupby, int& N, int L=19, int storagelimit=512, int nbofbitmap=32, char& delimiter = ',', int rng = rand_rng) 
{
  //int nbofbitmap = 32;
  vector<map< vector<T>, uint64> >buffer(nbofbitmap);
  ...
  for(int k = 0; k < nbofbitmap; ++k) {
       ...
       if(stoverbose) cout <<  buffer[k].size() << " "<< t[k]<<endl;
  }
...
*/
template <class T>
uint64 StoWeightedSumGibbonsTirthapura(istream & in, vector<string> & groupby, int& N, int L=19, int storagelimit=512, int nbofbitmap=32, char& delimiter = ',', int rng = rand_rng) 
{
  //int nbofbitmap = 32;
  vector<map< vector<T>, uint64> >buffer(nbofbitmap);
  ...
  while (in.getline(chline,MAXLENGTHOFALINE,delimiter))
  {
    ...
    //buffer[attribute_value] = (1LL << 32) | trailingzeros(oldhash,L);  // count= 1 
    uint projection = oldhash & mask[alpha];
    //cout << "alpha = "<< alpha << " projection = " << oldhash<< endl;
    /*if(trailingzeros(oldhash,L) >= t[alpha]) {
      // better have projection 0
      if(! (projection == 0)) cout << "You are in trouble!"<< endl;
    } else {
      if( (projection == 0)) cout << "You are in trouble! (2)"<< endl;
    }*/
    if (projection == 0) 
    {
      typename map< vector<T>,uint64>::iterator it = buffer[alpha].find(attribute_value); 
  ...
  for(int k = 0; k < nbofbitmap; ++k) {
       ...
       //if(stoverbose) cout << counts[k] << " "<< t[k]<<endl;
  }
  ...
  if(stoverbose) cout << "totalt"  << totalt <<endl;
  return (uint) ( (totalcount/phi) * ((pow(2, (totalt / nbofbitmap)))));
  /*
  double ratio = 0.0;
  for(int k = 0; k < nbofbitmap; ++k) {
     ratio += 1.0 / pow(2.0,t[k]) / nbofbitmap;
   }
  return (uint64) round(totalcount * 1/ratio);*/
}
