...
int main(int params, char ** args)  
{
	...
	if (params <= 3) 
	{
		cerr << "You must provide some parameters including  a sample size and a filename " << endl;
	...
	uint ec = n;
	if (n==0) ec = (uint)(p*N/100);//if -n is not set


	while (randomnumber.size() < ec)
	{
		int randomvalue = 1 + (int)((double)rand() / ((double)RAND_MAX + 1) * N);
		//cout << " -indice- " << randomnumber.size() << " -randomvalue- "<< randomvalue <<endl;
		randomnumber.insert(make_pair(randomvalue, true));
	}
	/*map<int,bool>::iterator i =  randomnumber.begin();
	while (i != randomnumber.end())
	{
		cout <<i->first << " ";
		++i;
	}
