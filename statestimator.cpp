...
//N, mmax and F0 are respectively the sample size, the maximal multiplicity and the number of distinct values
void EstimatePK(const int N, const int mmax,const  double F0, double &p, int &k)
{
	double F(0);
  assert(F < F0);
	k = (int) (log2 (F0));
  ...
  double oldF(F);
    ...
    if(F > F0) {
      if(abs(F-F0) < abs(oldF-F0)) {
        assert(F > F0);
        //cout << "#(final values) F0 =  "<< F0 << " F = " << F <<  endl;
        return;
      } else {
        F=oldF;
        assert(F <= F0);
...
template <class T>
uint64 MultiFractalEstimation(istream & in, vector<string> & groupby, const int N,  const char delimiter = ',') 
{
	...
	//cout << "P " << p << " K " << k <<endl;
	//Estimate from the population ==> Extrapolations from a sample of Nsample records, out of the total N records
...
int main(int params, char ** args) 
{
	...
	for(int i = 1; i < params - 1; ++i) 
	{
		...
		{
			N = atoi(args[++i]);
			if (verbose == true) cout << "Total number of facts: " << N << endl;
		}
	...
	if (verbose==true) cout << "Data file name:  " << datafilename.c_str() << endl;
	if (!filein) {
